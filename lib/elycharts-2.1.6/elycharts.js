			...
			*/
			...
			features: {
				...
				highlight: {
					...
					indexHighlightProps: {
						opacity: 1 /*fill : 'yellow', opacity : .3, scale : ".5 1"*/
					}
				},
				animation: {
...
(function($) {
	...
	$.elycharts.common = {
		...
		_absrectpath: function(x1, y1, x2, y2, r) {
			if (r) {
				...
				var res = [
					['M', x1, y1 + r],
					['a', r, r, 0, 0, 1, r, -r], //['Q',x1,y1, x1+r,y1],
					['L', x2 - r, y1],
					['a', r, r, 0, 0, 1, r, r], //['Q',x2,y1, x2,y1+r],
					['L', x2, y2 - r],
					['a', r, r, 0, 0, 1, -r, r], // ['Q',x2,y2, x2-r,y2],
					['L', x1 + r, y2],
					['a', r, r, 0, 0, 1, -r, -r], // ['Q',x1,y2, x1,y2-r],
					['z']
				];
				return res;
			} else return [
		...
		movePath: function(env, path, offset, marginlimit, simple) {
			...
			if (path.length == 1 && path[0][0] == 'RECT')
				return [
					[path[0][0], this._movePathX(env, path[0][1], offset[0], marginlimit), this._movePathY(env, path[0][2], offset[1], marginlimit), this._movePathX(env, path[0][3], offset[0], marginlimit), this._movePathY(env, path[0][4], offset[1], marginlimit), path[0][5]]
				];
			if (path.length == 1 && path[0][0] == 'SLICE') {
				if (!simple) {
					...
					return [
						[path[0][0], x, y, path[0][3], path[0][4], path[0][5], path[0][6]]
					];
				} else
					return [
						[path[0][0], path[0][1] + offset[0], path[0][2] + offset[1], path[0][3], path[0][4], path[0][5], path[0][6]]
					];
			}
			if (path.length == 1 && path[0][0] == 'CIRCLE')
				return [
					[path[0][0], path[0][1] + offset[0], path[0][2] + offset[1], path[0][3]]
				];
			if (path.length == 1 && path[0][0] == 'TEXT')
				return [
					[path[0][0], path[0][1], path[0][2] + offset[0], path[0][3] + offset[1]]
				];
		...
		minDelta:70,
		hidePoint:function(minDelta,env)
			{
				var obj={};
				var n = 1;
				do
				{
					var labelLength = Math.ceil(env.opt.labels.length/n) 
					var deltaX = (env.width - env.opt.margins[3] - env.opt.margins[1]) / (labelLength> 1 ? labelLength - 1 : 1);
					if(deltaX<minDelta)
					{
						n++;
					}
				}while(deltaX<minDelta);
				obj.n=n;
				obj.deltaX=deltaX;
				return obj;
			},
		...
		_animationStackAnimateElement: function(a, awith, awithanim) {
			...
			if (a.force || !a.piece.animationInProgress) {
				...
				if (!a.props)
					a.props = {
						opacity: 0
					}; // TODO Sarebbe da rimuovere l'elemento alla fine
	...
	$.elycharts.featuresmanager = {
		...
		init: function() {
			$.elycharts.featuresmanager.managers.sort(function(a, b) {
		...
		onMouseOver: function(env, serie, index, mouseAreaData) {
			var n = common.hidePoint(common.minDelta,env).n;
			if(index%n==0 || !serie.startsWith('limit'))
			{
				if(serie.startsWith('limit'))
				{
					index=index/n;
		...
		onMouseClick: function(env, serie, index, mouseAreaData) {
			var n = common.hidePoint(common.minDelta,env).n;
			if(index%n==0 || !serie.startsWith('limit'))
			{
				if(serie.startsWith('limit'))
				...
				if (!$.elycharts.featuresmanager.initialized)
					this.init();
				for (var i = 0; i < $.elycharts.featuresmanager.managers.length; i++)
					if ($.elycharts.featuresmanager.managers[i][1].onMouseClick)
						$.elycharts.featuresmanager.managers[i][1].onMouseClick(env, serie, index, mouseAreaData);
			}
		},
			...
			for (var i = 0; i < env.mouseAreas.length; i++) {
				...
				if (anc && env.mouseAreas[i].props.anchor && env.mouseAreas[i].props.anchor.highlight) {
					...
					(function(env, mouseAreaData, anc, caller) {
						...
						var f1 = function() {
							caller.anchorMouseOver(env, mouseAreaData);
						};
						var f2 = function() {
							caller.anchorMouseOut(env, mouseAreaData);
						};
	...
	$.elycharts.highlightmanager = {
		...
		onMouseOver: function(env, serie, index, mouseAreaData) {
			...
			// Il loop sotto estrae solo i pieces con array di path (quindi non i line o i fill del linechart ... ma il resto si)
				if (mouseAreaData.pieces[i].section == 'Series' && mouseAreaData.pieces[i].paths && (!serie || mouseAreaData.pieces[i].serie == serie) && mouseAreaData.pieces[i].paths[index] && mouseAreaData.pieces[i].paths[index].element) {
					...
					if (path && props.highlight) {
						if (props.highlight.scale) {
							...
							if (path[0][0] == 'RECT') {
								...
								path = [
									['RECT', path[0][1], path[0][2] - h * (scale[1] - 1), path[0][3] + w * (scale[0] - 1), path[0][4]]
								];
								...
								if (d > 90)
									d = 90;
								path = [
									['SLICE', path[0][1], path[0][2], path[0][3] * scale[0], path[0][4], path[0][5] - d, path[0][6] + d]
								];
								common.animationStackPush(env, piece, element, common.getSVGProps(env, path), props.highlight.scaleSpeed, props.highlight.scaleEasing);
			...
			if (env.opt.features.highlight.indexHighlight && env.opt.type == 'line') {
				...
				switch (t) {
					case 'bar':
						path = [
							['RECT', env.opt.margins[3] + index * delta1, env.opt.margins[0],
					...
					case 'barline':
						...
						path = [
							['M', x, env.opt.margins[0]],
							['L', x, env.height - env.opt.margins[2]]
						];
				}
				if (path) {
					//BIND: mouseAreaData.listenerDisabled = true;
					...
					env.highlighted.push({
						element: element,
						attr: false,
						cfg: env.opt.features.highlight
					});
				}
			}
		},
...
(function($) {
	...
	$.elycharts.legendmanager = {
		...
		afterShow: function(env, pieces) {
			...
			if (!env.opt.legend || env.opt.legend.length == 0 || env.opt.values.serie1==null)
				return;
			...
			for (serie in env.opt.legend) {
				...
				for (var j = 0; j < data.length; j++) {
					...
					if (color) {
						common.colorize(env, computedProps, [
							['dotProps', 'fill']
						], color);
					}
					...
					if (computedProps.dotType == "rect") {
						items.push(common.showPath(env, [
							['RECT', props.dotMargins[0] + x, y + Math.floor((h - computedProps.dotHeight) / 2), props.dotMargins[0] + x + computedProps.dotWidth, y + Math.floor((h - computedProps.dotHeight) / 2) + computedProps.dotHeight, computedProps.dotR]
						]).attr(computedProps.dotProps));
						xd = props.dotMargins[0] + computedProps.dotWidth + props.dotMargins[1];
					} else if (computedProps.dotType == "circle") {
						items.push(common.showPath(env, [
							['CIRCLE', props.dotMargins[0] + x + computedProps.dotR, y + (h / 2), computedProps.dotR]
						]).attr(computedProps.dotProps));
						xd = props.dotMargins[0] + computedProps.dotR * 2 + props.dotMargins[1];
					}

					var text = data[j];
					var t = common.showPath(env, [
						['TEXT', text, x + xd, y + Math.ceil(h / 2) + (Raphael.VML ? 2 : 0)]
					]).attr({
						"text-anchor": "start"
					}).attr(computedProps.textProps); //.hide();
					items.push(t);
			...
			var borderPath = [
				['RECT', propsx, props.y, propsx + propswidth, props.y + props.height, props.r]
			];
...
(function($) {
	...
	$.elycharts.mousemanager = {
		...
		afterShow: function(env, pieces) {
			...
			env.mouseLayer = $('<div></div>').css({
				position: 'absolute',
				'z-index': 20,
				opacity: 1
			}).prependTo(env.container);
			...
			if (env.opt.features.debug.active && typeof DP_Debug != 'undefined') {
				env.paper.text(env.width, env.height - 5, 'DEBUG').attr({
					'text-anchor': 'end',
					stroke: 'red',
					opacity: .1
				});
				paper.text(env.width, env.height - 5, 'DEBUG').attr({
					'text-anchor': 'end',
					stroke: 'red',
					opacity: .1
				}).click(function() {
			...
			if (env.opt.features.mousearea.type == 'single') {
					...
					if (pieces[i].mousearea) {
						...
						if (!pieces[i].paths) {
							...
							if (pieces[i].path.length >= 1 && (pieces[i].path[0][0] == 'LINE' || pieces[i].path[0][0] == 'LINEAREA'))
								for (j = 0; j < pieces[i].path[0][1].length; j++) {
									...
									if (props.mouseareaShowOnNull || pieces[i].section != 'Series' || env.opt.values[pieces[i].serie][j] != null)
										env.mouseAreas.push({
											path: [
												['CIRCLE', pieces[i].path[0][1][j][0], pieces[i].path[0][1][j][1], 10]
											],
											piece: pieces[i],
											pieces: pieces,
											index: j,
							...
							// TODO DELETE
								...
								env.mouseAreas.push({
									path: [
										['CIRCLE', common.getX(pieces[i].path[j]), common.getY(pieces[i].path[j]), 10]
									],
									piece: pieces[i],
									pieces: pieces,
									index: j,
				...
				for (var idx in env.opt.labels) {
					...
					env.mouseAreas.push({
						path: [
							['RECT', start + index * delta, env.height - env.opt.margins[2], start + (index + 1) * delta, env.opt.margins[0]]
						],
						piece: false,
						pieces: pieces,
			...
			if (!env.opt.features.mousearea.syncTag) {
				...
				if (!$.elycharts.mouseareaenv[env.opt.features.mousearea.syncTag])
					$.elycharts.mouseareaenv[env.opt.features.mousearea.syncTag] = {
						chartEnv: false,
						mouseObj: false,
						caller: false,
						inArea: -1,
			...
			for (i = 0; i < env.mouseAreas.length; i++) {
				env.mouseAreas[i].area = common.showPath(env, env.mouseAreas[i].path, paper).attr({
					stroke: "#000",
					fill: "#fff",
					opacity: 0
				});
				...
				(function(env, obj, objidx, caller, syncenv) {
					...
					obj.mouseclick = function(e) {
						caller.onMouseClickArea(env, piece, index, obj);
					}
					...
					$(obj.area.node).click(obj.mouseclick);
		...
		onMouseClickArea: function(env, piece, index, mouseAreaData) {
			if (env.opt.features.mousearea.onMouseClick)
				env.opt.features.mousearea.onMouseClick(env, mouseAreaData.piece ? mouseAreaData.piece.serie : false, mouseAreaData.index, mouseAreaData);
			featuresmanager.onMouseClick(env, mouseAreaData.piece ? mouseAreaData.piece.serie : false, mouseAreaData.index, mouseAreaData);
		},
		...
		_prepareShow: function(env, props, mouseAreaData, tip) {
			...
			if (props.width && props.width != 'auto' && props.height && props.height != 'auto') {
				...
				if (!env.tooltipFrameElement && props.frameProps) {
					var framePath = [
						['RECT', delta / 2, delta / 2, props.width, props.height, props.roundedCorners]
					];
...
(function($) {
	...
	$.elycharts.line = {
		...
		draw: function(env) {
			...
			var limits = env.opt.limits;
			var serie1 =  values[Object.keys(values)[0]];
			var length =serie1!=null?serie1.length:0;
			...
			if (!labels)
				labels = [];
			// while (labelsCount > labels.length) // Commented
			//   labels.push(null);
			var fromVector = env.plots.serie1.from;
			labelsCount = fromVector!=undefined?fromVector.length:0; //labels.length;
			...
			// DEP: *
			var deltaX = (env.width - opt.margins[3] - opt.margins[1]) / (labelsCount /*labels.length*/ > 1 ? labelsCount /*labels.length*/ - 1 : 1);
			var deltaBarX = (env.width - opt.margins[3] - opt.margins[1]) / (labelsCount /*labels.length*/ > 0 ? labelsCount /*labels.length*/ : 1);
			var n = common.hidePoint(common.minDelta,env).n;
			for (serie in values) {
				props = common.areaProps(env, 'Series', serie);
				if(serie.startsWith('limit') )
				...
				if (values[serie] && props.visible) {
					...
					if (props.type == 'line') {
						...
						var fillPath = ['LINEAREA', [],
							[], props.rounded
						];
						var dotPieces = [];

						for (i = 0, ii = labelsCount /*labels.length*/ ; i < ii; i++)
							if (plot.to.length > i) {
								...
								if (indexProps.dot && (!serie.startsWith("limit") || i%n==0)) {
									if (values[serie][i] == null && !indexProps.dotShowOnNull)
										...
										dotPieces.push({
											path: [
												['CIRCLE', x, y, indexProps.dotProps.size]
											],
											attr: indexProps.dotProps,
										}); // TODO Size should not be in dotProps (not an svg props)
						...
						for (i = 0, ii = labels.length; i < ii; i++)
							if (plot.to.length > i) {
								if (plot.from[i] != plot.to[i]) {
									...
									pieceBar.push({
										path: [
											['RECT', x1, y1, x1 + bwid - bpad * 2, y2]
										],
										attr: indexProps.plotProps
									});
								} else
									pieceBar.push({
										path: false,
		...
		grid: function(env, pieces) {
			...
			if (common.executeIfChanged(env, ['values', 'series', 'axis', 'labels', 'margins', 'width', 'height', 'features.grid'])) {
				...
				var axis = env.axis;
				var labels = [];
				var obj = common.hidePoint(common.minDelta,env);
				var n = obj.n;
				var deltaX = obj.deltaX;
				for(i=0;i<env.opt.labels.length;i=i+n)
				{
					labels.push(env.opt.labels[i]);
				}
				...
				if (axis.x && axis.x.props.title) {
					...
					pieces.push({
						...
						subSection: 'Title',
						path: [
							['TEXT', axis.x.props.title, x, y]
						],
						attr: axis.x.props.titleProps
					});
				} else
					pieces.push({
						section: 'Axis',
				...
				for (var jj in ['l', 'r']) {
					...
					if (axis[j] && axis[j].props.labels && props.ny) {
						...
						for (i = axis[j].props.labelsSkip ? axis[j].props.labelsSkip : 0; i <= props.ny; i++) {
							...
							else if(env.opt.percentage==true)
							{
								txt=txt+'%';
							}
							...
							paths.push({
								path: [
									['TEXT', txt, labx, laby + (axis[j].props.labelsMargin ? axis[j].props.labelsMargin : 0)]
								],
								attr: axis[j].props.labelsProps
							});
						}
						pieces.push({
							section: 'Axis',
				...
				if (props.nx || props.ny) {
					...
					pieces.push({
						...
						attr: path.length ? props.props : false
					});
					pieces.push({
						section: 'GridBandH',
						paths: bandsH
					...
					pieces.push({
						section: 'Ticks',
						path: tpath.length ? tpath : false,
						attr: tpath.length ? props.ticks.props : false
					});
				}
			}
		}
	...
	$.elycharts.pie = {
		...
		draw: function(env) {
			...
			for (serie in opt.values) {
				...
				common.colorize(env, serieProps, [
					['plotProps', 'stroke'],
					['plotProps', 'fill']
				], common.getItemColor(env, serie));
					...
					for (i = 0, ii = plot.values.length; i < ii; i++)
						if (plot.values[i] > 0) {
							...
							common.colorize(env, props, [
								['plotProps', 'stroke'],
								['plotProps', 'fill']
							], common.getItemColor(env, serie, i));
						...
						for (i = 0, ii = plot.values.length; i < ii; i++) {
							...
							if (value > 0) {
								...
								if (!env.opt.clockwise)
									paths.push({
										path: [
											['SLICE', cx, cy, rrend, rrstart, angle, angle + angleplus]
										],
										attr: props.plotProps
									});
								else
									paths.push({
										path: [
											['SLICE', cx, cy, rrend, rrstart, -angle - angleplus, -angle]
										],
										attr: props.plotProps
									});
							} else

	...
	$.elycharts.line = {
		...
		draw: function(env) {
			...
			do
			{
				...
				var deltaXLimit = (env.width - opt.margins[3] - opt.margins[1]) / (labelLength> 1 ? labelLength - 1 : 1);
				if(deltaXLimit<minDelta)
				{
					n++;
				}
			}while(deltaXLimit<minDelta);
			for (serie in values) {
				props = common.areaProps(env, 'Series', serie);
				if(serie.startsWith('limit') )
				{
		...
		grid: function(env, pieces) {
			...
			if (common.executeIfChanged(env, ['values', 'series', 'axis', 'labels', 'margins', 'width', 'height', 'features.grid'])) {
				...
				{
					var labelLength = Math.ceil(env.opt.labels.length/n) 
					var deltaX = (env.width - opt.margins[3] - opt.margins[1]) / (labelLength> 1 ? labelLength - 1 : 1);
					if(deltaX<minDelta)
					{
						n++;
					}
